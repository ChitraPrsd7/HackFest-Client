...
export class CallbackComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      if (!code) {
        // No code — could be an error response from IdP
        const idpError = params['error_description'] || params['error'] || 'Unknown error';
        console.error('[eSewa Callback] No auth code received:', idpError);
        this.error = `Login failed: ${idpError}`;
        setTimeout(() => this.router.navigate(['/auth/login']), 3000);
        return;
      }

      console.log('[eSewa Callback] Received code, exchanging for tokens…');

      this.oneIdService.handleCallback(code, this.clientId, this.redirectUri).subscribe({
        next: (tokens) => {
          console.log('[eSewa Callback] Token exchange successful:', tokens.token_type);
          this.router.navigate(['/dashboard']);
        },
        error: (err) => {
          console.error('[eSewa Callback] Token exchange failed:', err);
